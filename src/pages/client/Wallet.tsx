        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="gradient-primary rounded-2xl p-8 text-primary-foreground">
          ...
          <div className="flex gap-4 mt-6">
            <div className="bg-primary-foreground/20 rounded-lg px-4 py-2">
              ...
              <p className="font-semibold">${myTx.filter(t => t.type === 'refund').reduce((s, t) => s + t.amount, 0).toFixed(2)}</p>
            </div>
            <div className="bg-primary-foreground/20 rounded-lg px-4 py-2">
              <p className="text-xs opacity-80">Total Out</p>
              <p className="font-semibold">${myTx.filter(t => t.type === 'withdrawal' && t.status === 'completed').reduce((s, t) => s + t.amount, 0).toFixed(2)}</p>
            </div>
          </div>
        </motion.div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {format(new Date(t.createdAt), 'MMM dd, yyyy HH:mm')}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
